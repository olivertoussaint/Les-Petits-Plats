    ...
    li.addEventListener("click", (e) => {
      ...
      if (mainInputFilled === false) {
        if (
          ...
          if (tagsMap.size === 1) {
            tagsMap.forEach((itemTittleList, item) =>
              displayRecipes(
                filterRecipesByField(item, recipes, itemTittleList)
              )
            );
            tagsMap.forEach((itemTittleList, item) => {
    ...
    if (e.target.className.includes("far fa-times-circle")) {
      ...
      if (mainInputFilled === false) {
        if (tagsMap.size === 0) {
          ...
          tagsMap.forEach((itemTittleList, item) =>
            displayRecipes(filterRecipesByField(item, recipes, itemTittleList))
          );
          tagsMap.forEach((itemTittleList, item) => {
            filteredListsAdvancedField = renderItemsInDropdowns(
              filterRecipesByField(item, recipes, itemTittleList)
        ...
        if (tagsMap.size === 0) {
          ...
          tagsMap.forEach((itemTittleList, item) =>
            displayRecipes(
              filterRecipesByField(item, arrayFromMainInput, itemTittleList)
            )
          );
          tagsMap.forEach((itemTittleList, item) => {
            filteredListsAdvancedField = renderItemsInDropdowns(
